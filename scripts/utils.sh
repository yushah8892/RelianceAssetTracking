...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.reliance-network.com:7050 -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.upgrad-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.reliance-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.upgrad-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode upgrade -o orderer.reliance-network.com:7050 -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.upgrad-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode upgrade -o orderer.reliance-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n edtech -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.upgrad-network.edtech:instantiate"]}' -P "OR ('InfrastructureMSP.member','PowerMSP.member','CommunicationsMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.reliance-network.com:7050 -C $CHANNEL_NAME -n edtech $PEER_CONN_PARMS -c '{"Args":["org.upgrad-network.edtech:create","0001","Medicine","2","Capital-Comm"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.reliance-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n edtech $PEER_CONN_PARMS -c '{"Args":["org.upgrad-network.edtech:create","0001","Medicine","2","Capital-Comm"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
