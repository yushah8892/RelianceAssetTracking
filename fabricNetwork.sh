...
function generateChannelArtifacts() {
  ...
  echo "in chqannel artifect section"
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate ./channel-artifacts/PowerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg PowerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate ./channel-artifacts/CommunicationsMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg CommunicationsMSP
  res=$?0000
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate ./channel-artifacts/EntertainmentMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg EntertainmentMSP
  res=$?0000
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelFiveOrgs -outputAnchorPeersUpdate ./channel-artifacts/CapitalMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg CapitalMSP
  res=$?0000
  set +x
  if [ $res -ne 0 ]; then
...
COMPOSE_FILE_COUCHDB=docker-compose-couch.yaml
#
